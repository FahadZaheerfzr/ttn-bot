...
def isMembershipMarkup(order_id, currency):
    ...
    markup.add(
        types.InlineKeyboardButton(text="📲 Upgrade", callback_data=f"upgrade {order_id} {currency}"),
...
def sameSection(message: types.CallbackQuery, bot: TeleBot, order_id, currency):
    ...
    if not user_info:
        bot.reply_to(message.message.chat.id, "You are not registered with bot yet, Press /start to register!")
    ...
    newPrices = {"bnb": bnb_price, "ttn": ttn_price, "busd": prices['busd_price']}
    ...
    if currency == "bnb":
        ...
        token = web3.eth.contract(address=web3.toChecksumAddress(config.TTN_CONTRACT), abi=config.TTN_CONTRACT_ABI)
        balance = token.functions.balanceOf(user_info['address']).call() / 10 ** 9
    elif currency == "busd":
        token = web3.eth.contract(address=web3.toChecksumAddress(config.BUSD_CONTRACT), abi=config.TTN_CONTRACT_ABI)
        balance = token.functions.balanceOf(user_info['address']).call() / 10 ** 18
    else:
    ...
    if not 'owner_wallet' in owner_info:
        bot.answer_callback_query(message.id, "This Group Owner Didn't Set His/Her Address To Receive Payment, Please Contact Them & Ask To Set", show_alert=True)
        return None
...
def upgrade(message: types.CallbackQuery, bot: TeleBot):
    ...
    if not function_output: pass
    order_info, payment_contract, owner_info, new_price_type, user_info = function_output

    membership_info = DB['memberships'].find_one({"chat_id": int(order_info['chat_id']), "user_id": message.from_user.id})


    if currency == "ttn": return token_functions.acceptPaymentTTN(
    ...
    try:
        ...
        tx['nonce'] = web3.eth.getTransactionCount(web3.eth.account.privateKeyToAccount(user_info['private_key']).address)

        signedTx = web3.eth.account.sign_transaction(tx, private_key=user_info['private_key'])
...
• Group Link: {invite_link.invite_link}
    ...
    bot.edit_message_text(message_id=message.message.id, chat_id=message.message.chat.id, text="Membership Upgraded Successfully", reply_markup=None)

def pay(message: types.CallbackQuery, bot: TeleBot):
    command, order_id, currency = message.data.split(" ")
    function_output = sameSection(message, bot, order_id, currency)
    # print(function_output)
    if not function_output: return
    order_info, payment_contract, owner_info, new_price_type, user_info = function_output

    is_membership = DB['memberships'].find_one({"chat_id": int(order_info['chat_id']), "user_id": message.from_user.id})
    if is_membership:
        if is_membership['is_one_time']: return bot.answer_callback_query(message.id, "You already have purchase lifetime susbcription", show_alert=True)
