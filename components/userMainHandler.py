...
def middlewareMainHandler(message: types.CallbackQuery, bot: TeleBot):
    ...
    if command == "help": return helpHandler(message, bot)
    if command == "email": return emailHandler(message, bot)
    if command == "wallet": return walletHandler(message, bot)
    if command == "notification": return notificationHandler(message, bot)
    if command == "subscription": return subscriptionHandler(message, bot)
    if command == "communities": return communitiesHandler(message, bot)
    if command == "toggleEmail": return toggleEmailHandler(message, bot)
    if command == "toggleTg": return toggleTgHandler(message, bot)
...
def notificationHandler(message: types.CallbackQuery, bot: TeleBot):
    ...
    markup.add(
        types.InlineKeyboardButton(text=current_mail, callback_data="main_toggleEmail"),
        types.InlineKeyboardButton(text=current_tg, callback_data="main_toggleTg"),
    )
    markup.add(
...
def toggleTgHandler(message: types.CallbackQuery, bot: TeleBot):
    ...
    if ("tg_not" in userInfo) :
        DB['users'].update_one({"_id": message.from_user.id}, {"$unset": {"tg_not": ""}})
    else :
        DB['users'].update_one({"_id": message.from_user.id}, {"$set": {"tg_not": True}})
    ...
    markup.add(
        types.InlineKeyboardButton(text="🔙 Back To Menu", callback_data="main_backToMenu"),
        types.InlineKeyboardButton(text="🔙 Back To Notification", callback_data="main_notification"),
    )
...
def toggleEmailHandler(message: types.CallbackQuery, bot: TeleBot):
    ...
    if ("email_not" in userInfo) :
        DB['users'].update_one({"_id": message.from_user.id}, {"$unset": {"email_not": ""}})
    else :
        DB['users'].update_one({"_id": message.from_user.id}, {"$set": {"email_not": True}})
...
def subscriptionHandler(message: types.CallbackQuery, bot: TeleBot):
    ...
    for membership in memberships :
        ...
        markup.add(
            types.InlineKeyboardButton(text=group_info["name"], url=invite_link.invite_link),
        )
...
def communitiesHandler(message: types.CallbackQuery, bot: TeleBot):
    ...
    for community in communities :
        markup.add(
            types.InlineKeyboardButton(text=f"{community['name']}", callback_data=f"community_{community['_id']}"),
        )
...
def walletHandler(message: types.CallbackQuery, bot: TeleBot):
    ...
    text_to_send = f"""
<i>💵 Your Wallet Info :</i>

<i>• Address :|</i> {userInfo['address']}
...
def emailHandlerCallback(message: types.Message, bot:TeleBot):
    email = message.text
    if email.lower() == "cancel": return start.start(message, bot)
    ...
    DB['users'].update_one({"_id": message.from_user.id}, {"$set": {"email": email}})
    bot.send_message(message.chat.id, "<b>Email Updated Successfully</b>", parse_mode='HTML')
